...
//
   ...
   function controle()
   {
      ...
      if ($this->NM_ajax_flag && 'event_' == substr($this->NM_ajax_opcao, 0, 6))
      {
          $this->nm_tira_formatacao();
          $this->nm_converte_datas();
          if ('event_detalle_movilizacion_destino__onchange' == $this->NM_ajax_opcao)
          {
              $this->detalle_movilizacion_destino__onChange();
          }
   ...
   { 
      ...
      if ($this->nmgp_opcao == "alterar") 
      {
          ...
          $this->detalle_movilizacion_destino_ = $this->detalle_movilizacion_destino__before_qstr;
              ...
              $this->nmgp_refresh_fields = array_diff(array('detalle_movilizacion_origen_', 'detalle_movilizacion_destino_', 'detalle_movilizacion_distancia_', 'detalle_movilizacion_fecha_inicio_', 'detalle_movilizacion_fecha_fin_', 'iddetalle_movilizacion_'), $aDoNotUpdate);
              $this->ajax_return_values();
...
//
function Calcular_Distancia($loc_orig, $loc_dest)
{
$_SESSION['scriptcase']['form_detalle_movilizacion']['contr_erro'] = 'on';
  
$apiKey = "AIzaSyAdvmWWi-df4qQU77pnu_UyvZJ5bFijWeo";
$ApiKeyDistanceMatrix = "AIzaSyAQ-4v6GEibbY1o5XAQBiEo8eJkQq-33dM";
...
$params = array(
	...
	'region' => 'en' , 
	'origins'=>$loc_orig,
	'destinations'=>$loc_dest,
	'key'=> $ApiKeyDistanceMatrix
);
...
function detalle_movilizacion_destino__onChange()
{
$_SESSION['scriptcase']['form_detalle_movilizacion']['contr_erro'] = 'on';
...
$origen = "'".$this->detalle_movilizacion_origen_ .",Ecuador,0,0'";
$destino = "'".$this->detalle_movilizacion_destino_ .",Ecuador,0,0'";
...
$response = sc_webservice('file_get_contents', $googleUrl.$params, $_SERVER["SERVER_PORT"], "GET", "", array(), 30);

$dados = json_decode($response);
if($dados->status == "OK"){
	if($dados->rows[0]->elements[0]->status == "OK"){
		$distancia_km = $dados->rows[0]->elements[0]->distance->text; 
		$distancia = str_replace(" km","",$distancia_km);
		$this->detalle_movilizacion_distancia_  = $distancia;

	}else{
		
 if (!isset($this->Campos_Mens_erro)){$this->Campos_Mens_erro = "";}
 if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= "Error de cálculo de movilizacion";
 if ('submit_form' == $this->NM_ajax_opcao || 'event_' == substr($this->NM_ajax_opcao, 0, 6))
 {
  $sErrorIndex = ('submit_form' == $this->NM_ajax_opcao) ? 'geral_form_detalle_movilizacion' : substr(substr($this->NM_ajax_opcao, 0, strrpos($this->NM_ajax_opcao, '_')), 6);
  $this->NM_ajax_info['errList'][$sErrorIndex][] = "Error de cálculo de movilizacion";
 }
;
		if (isset($this->Campos_Mens_erro) && !empty($this->Campos_Mens_erro))
{
 ...
 if (!isset($this->Campos_Mens_erro)){$this->Campos_Mens_erro = "";}
 if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= "Error de consulta de destino y origen";
 if ('submit_form' == $this->NM_ajax_opcao || 'event_' == substr($this->NM_ajax_opcao, 0, 6))
 {
  $sErrorIndex = ('submit_form' == $this->NM_ajax_opcao) ? 'geral_form_detalle_movilizacion' : substr(substr($this->NM_ajax_opcao, 0, strrpos($this->NM_ajax_opcao, '_')), 6);
  $this->NM_ajax_info['errList'][$sErrorIndex][] = "Error de consulta de destino y origen";
 }
;
	if (isset($this->Campos_Mens_erro) && !empty($this->Campos_Mens_erro))
{
...
$distancia = "SELECT SUM(detalle_movilizacion_distancia)
FROM detalle_movilizacion
WHERE detalle_movilizacion.Id_Movilizacion ='".$this->id_movilizacion_ ."'";
