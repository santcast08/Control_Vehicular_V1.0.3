<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idmantenimiento_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idmantenimiento_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_mantenimiento_impresion"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function sc_btn_imprimir_mantenimiento() 
{ 
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_imprimir_mantenimiento(); }, 500);
      return;
    }
    window.print();
} 
...
{ 
    ...
    Nm_Proc_Atualiz = true;
    do_ajax_form_mantenimiento_impresion_submit_form(y, z); 
} 
<?php
}
?>
...
var nm_num_campo = 1;
function lookup_idvehiculo_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.idvehiculo_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_mantenimiento_impresion')?>/form_mantenimiento_impresion.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&script_case_session=<?php echo session_id() ?>&nmgp_opcao=lookup", recebe_idvehiculo_, "idvehiculo_", dados, false);
}
function recebe_idvehiculo_(string)
{
   document.getElementById('id_lookup_idvehiculo_' + nm_num_campo).innerHTML = string;
}
<?php 
